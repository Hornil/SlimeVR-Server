...
## Tracker status
tracker-status-none = Нет статуса
tracker-status-busy = Занято
...
## Tracker mechanics
settings-general-tracker_mechanics = Механика трекеров
...
onboarding-automatic_proportions-recording-steps-4 = Покачивайтесь, пока не закончится таймер.
...
onboarding-automatic_proportions-verify_results-processing = Обрабостка результатов
